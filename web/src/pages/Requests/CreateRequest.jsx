...
import {
  X, Save, Info, Upload, Plus, HelpCircle,
  LayoutGrid, Image as ImageIcon, Video, FileArchive, CheckCircle
} from 'lucide-react';
...
export default function CreateRequestModal({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (open) {
      setForm({ code: '', productTypes: [], videoQuality: '', priority: '', deadline: '', quantity: 1, splitByImage: false, notes: '' });
      setImages([]); setErrors({}); setSaved(false); setSaving(false);
    }
  }, [open]);

  // ESC không đóng modal (chỉ đóng bằng nút X hoặc Hủy)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validate()) return;
    setSaving(true);
    const newReq = await requestApi.create({ ...form, createdBy: user.name, status: 'pending', title: `Đơn ${form.code}`, client: '—' });
  ...
  return (
    <>
      {/* Backdrop — click outside KHÔNG đóng */}
      <div
        style={{
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '4px 24px' }}>
          {saved ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 0', gap: 14 }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 700 }}>Tạo đơn hàng thành công!</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Đang đóng...</div>
            </div>
          ) : (
